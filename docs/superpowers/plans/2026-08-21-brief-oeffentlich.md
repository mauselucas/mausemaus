...
const v = JSON.parse(await s.werte(`(async () => {
  ...
  const k = document.querySelector('.mm-vorschau');
...
```
/welt/:slug   /welt.html   200
/blog/:slug   /welt.html   200
```

**`:slug` statt `*` ist wichtig, nicht Geschmackssache.** `:slug` trifft genau **einen**
Adressabschnitt. Mit `*` würde die Regel auch `/welt/assets/fonts.css` einfangen und dafür
`welt.html` mit Erfolgsmeldung ausliefern — ein fehlender oder falsch geschriebener Pfad
käme also nie als Fehler an, sondern als HTML, das sich als Stylesheet ausgibt. Genau so
ist auf dieser Website schon einmal eine Seite ohne jede Gestaltung live gegangen, ohne
dass es jemandem auffiel. Mit `:slug` schlägt so etwas laut fehl statt leise.
